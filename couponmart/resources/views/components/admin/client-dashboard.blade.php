...
<div class="profile-sidebar p-3 bg-white text-start" style="width:250px;">
    ...
    <div id="kats" class="hides">
        
        @foreach([
           
           ['route'=>'dashboard','title'=>'Deposit','icon'=>'bx bx-coin-stack'],
           ['route'=>'dashboard', 'title'=>'Withdrawal','icon'=>'bx bx-money-withdraw'],
           ['route'=>'wallet.index', 'title'=>' Details ','icon'=>'bx bxs-coupon'],
           
           ] as $sidebar)            
           
               <a class="kategory" href="{{ route($sidebar['route']) }}"><i class="{{$sidebar['icon']}}"></i>{{$sidebar['title']}}</a>
