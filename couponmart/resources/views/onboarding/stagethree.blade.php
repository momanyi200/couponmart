...
<div class="max-w-6xl mx-auto mb-8 pt-16">
    <div class="w-full text-center">
        <h3 class="logsubs text-2xl font-semibold">
            {{ isset($updateMode) && $updateMode ? 'Update Business Logo' : 'Step 3: Upload Business Logo' }}
        </h3>
        <span class="text-gray-600">
            {{ isset($updateMode) && $updateMode ? 'Update your current business logo below.' : 'Use the form below to sign up for the site' }}
        </span>
    </div>
</div>
...
<div class="max-w-6xl mx-auto flex justify-center items-center min-h-[45vh]">
    <div class="w-full max-w-2xl">
        <form 
            action="{{ isset($updateMode) && $updateMode ? route('profile.update-image') : route('onboarding.stage3.store') }}" 
            method="POST" 
                ...
                <div class="flex flex-wrap gap-6">
                    ...
                    <div class="flex flex-col justify-center w-full max-w-sm space-y-4 mx-auto">
                        ...
                        <button type="submit" class="bg-blue-600 text-white py-2 px-4 rounded shadow hover:bg-blue-700 transition image-upload">
                            {{ isset($updateMode) && $updateMode ? 'Update Image' : 'Upload Image' }}
                        </button>
                    </div>
                </div>
            </div>
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $('.image-upload').on('click', function (ev) { 
            ev.preventDefault(); // prevent double submit
            resize.croppie('result', { type: 'canvas', size: 'viewport' })
                .then(function (img) {
